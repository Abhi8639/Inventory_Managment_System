...
function WarehouseManagement() {
  ...
  const handleAddOrUpdateWarehouse = async () => {
    try {
      if (editMode) {
        ...
        await axios.put(`http://localhost:8080/api/warehouses/update/${editWarehouseId}`, newWarehouse);
      } else {
        // Adds a new warehouse
        await axios.post('http://localhost:8080/api/warehouses/add', newWarehouse);
      }
  ...
  const handleDeleteWarehouse = async (warehouseId) => {
    try {
      await axios.delete(`http://localhost:8080/api/warehouses/delete/${warehouseId}`);
      fetchWarehouses(); // Refreshes the warehouse list
    } catch (error) {
