...
/**
 * AllOrdersPage component displays a list of orders and their details,
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            try {
                ...
                // Sort orders in descending order based on the numeric part of orderId
                const sortedOrders = Array.isArray(ordersResponse.data)
                    ? ordersResponse.data.sort((a, b) => {
                        const aNum = parseInt(a.orderId.replace(/\D/g, ''), 10); // Extract numeric part
                        const bNum = parseInt(b.orderId.replace(/\D/g, ''), 10); // Extract numeric part
